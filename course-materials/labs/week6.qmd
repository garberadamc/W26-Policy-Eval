...
The relevance condition is testable (we can check whether $Z$ predicts $D$ in a regression). The exclusion restriction is **not directly testable** and must be argued on theoretical or contextual grounds.
...
$$Y = \beta_0 + \beta_1 \hat{D} + \beta_2 W + \varepsilon$$

The coefficient $\beta_1$ is the IV estimate of the causal effect of $D$ on $Y$. Because $\hat{D}$ is constructed only from variation in $Z$ (and $W$), it is purged of the endogenous component.
...
If we simply regress our measure of respiratory illness ($Y$) on PM2.5 concentrations ($X$), our estimate will likely be biased. Consider the potential sources of endogeneity:
...
We'll use California 2023 data for PM2.5 and precipitation from EPA's AQS. We filter to LA County (county code `"037"`).
...
Using the predicted values ($\hat{X}$) from the first stage, we now estimate the second stage. This isolates the variation in PM2.5 that is driven *only* by precipitation ($Z$).
...
:::{.callout-warning}
Remember: the standard errors from this manual second stage are **not correct** for inference. They don't account for the estimation uncertainty in $\hat{X}$ from the first stage. Always use `ivreg()` for your actual results.
:::
